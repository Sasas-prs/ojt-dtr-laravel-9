<x-main-layout>
        ...
        <div class="flex lg:!flex-row flex-col justify-between gap-5 items-center">
            ...
            <div class="flex">
                ...
                @foreach ($tabs as $key => $label)
                    <a href="?status={{ $key }}"
                        class="px-4 py-2 {{ $activeTab === $key ? 'border-b-2 border-custom-orange text-custom-orange font-semibold' : 'text-gray-500 hover:text-black' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
    ...
    <script>
        ...
        function viewRequest(requestId, month, year) {
            window.location.href = `/dtr-view/${requestId}?month=${month}&year=${year}&type=view`;
        }

        function downloadRequest(requestId, month, year) {
            window.location.href = `/dtr-view/${requestId}?month=${month}&year=${year}&type=download`;
        }
